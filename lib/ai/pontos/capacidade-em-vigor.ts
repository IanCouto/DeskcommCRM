...
/**
 * A consulta ao catálogo, num lugar só.
 *
 * `supports_vision` é `not null default false` no schema, então `null` aqui
 * significa **não há linha para este modelo** — e não "o provedor declarou que
 * não enxerga". A distinção é o que faz `enxergaImagem` cair no registro em vez
 * de afirmar `false` sobre um modelo que ninguém cadastrou.
 */
/**
 * O mínimo que esta consulta precisa do client — e não o `SupabaseClient`
 * inteiro. O builder do PostgREST é *thenable*, não `Promise` (não tem
 * `catch`/`finally`), então declarar `Promise` aqui reprovaria o client real;
 * `PromiseLike` é o que ele de fato satisfaz. Interface estreita também deixa o
 * teste passar um dublê de três linhas em vez de fingir um client.
 */
export interface LeitorDoCatalogoDeModelos {
  from(tabela: "ai_models"): {
    select(colunas: "supports_vision"): {
      eq(
        coluna: "provider",
        valor: string,
      ): {
        eq(
          ...
          is(
            coluna: "deprecated_at",
            valor: null,
          ): { maybeSingle(): PromiseLike<{ data: { supports_vision: boolean | null } | null }> };
        };
      };
    };
  };
...
export function visaoDeclaradaNoCatalogo(
  db: LeitorDoCatalogoDeModelos,
  provider: string,
  modelId: string,
