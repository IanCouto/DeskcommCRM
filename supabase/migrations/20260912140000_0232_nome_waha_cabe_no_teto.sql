-- 0232 — o nome da sessão WAHA tem que caber em 54 caracteres.
--
-- O POST /api/sessions do WAHA 2026.7.2 valida `name` com MaxLength(54).
-- A reserva da 0228/0230 gerava org_ + uuid-da-org-sem-hífen (32) + _ +
-- uuid-novo-sem-hífen (32) = 69. O create devolvia HTTP 400 e a tela
-- respondia connection_repair_required — o QR nunca aparecia.
-- Formato novo: org_ + org sem hífen (32) + _ + 16 hex = 53.
-- Canal WORKING não se renomeia: se alguém pareou num WAHA sem o teto,
-- trocar o nome desconecta o aparelho.
update public.channel_sessions
   set waha_session_name = 'org_'||replace(organization_id::text,'-','')||'_'||substr(md5(id::text),1,16)
 where waha_session_name is not null
   and char_length(waha_session_name) > 54
...
begin
 ...
 if found then
  ...
  if channel.id is null then
   ...
   values(p_org,'org_'||replace(p_org::text,'-','')||'_'||substr(md5(gen_random_uuid()::text),1,16),p_display_name,'NOWEB',
 ...
 if channel.waha_session_name is not null
    and char_length(channel.waha_session_name) > 54
    and channel.status is distinct from 'WORKING' then
  update public.channel_sessions
     set waha_session_name='org_'||replace(organization_id::text,'-','')||'_'||substr(md5(id::text),1,16)
   where organization_id=p_org and id=channel.id and status is distinct from 'WORKING'
